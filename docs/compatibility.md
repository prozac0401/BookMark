...
> 2026-09-22 WorkBookmark 0.2.2: 스티커의 메모 텍스트 영역을 한 번 클릭하면 같은 스티커에서 바로 편집합니다. Release 빌드 경고 0·오류 0, Desktop 회귀검사 197개 통과, 합성 화면 4개를 확인했습니다. [이번 검증 기록](evidence/note-click-2026-09-22/README.md)과 [릴리스 기록](release-notes-v0.2.2.md)을 참고하세요. 변경하지 않은 Core·어댑터·IPC 등은 이번에 재실행하지 않았으며, 아래 0.2.1 결과는 이전 버전의 이력입니다. 패키지는 미서명이며 실제 설치·마우스 입력·한글 IME·여러 모니터 실기는 미실행입니다.

> 2026-09-22 WorkBookmark 0.2.1: 포스트잇 최상단 표시, 해당 스티커만 처리 표시, 스티커 안 메모 편집과 파일 종류 아이콘을 반영했습니다. [릴리스 기록](release-notes-v0.2.1.md)과 [수정 및 실제 검증 범위](sticker-fixes-v0.2.1.ko.md)를 참고하세요. 자동검사 586개·MSI 구조 검사 54개·합성 화면 7개를 확인했습니다. 실제 설치·업데이트·재부팅, 한글 IME·여러 모니터 실기는 미실행이며 기존 환경별 확인 범위는 그대로 유지합니다.
...
아래 표의 검증일은 2026-09-19입니다. 최신 변경과 확인 결과는 [0.2.2 릴리스 기록](release-notes-v0.2.2.md), 이전 스티커 개선 결과는 [0.2.1 수정·검증 기록](sticker-fixes-v0.2.1.ko.md)을 참고하세요. 소스 구현과 실제 환경 지원을 구분합니다.

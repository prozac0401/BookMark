...
The temporary input observer retains only a boolean for fresh keyboard presses, mouse buttons and wheel activity. Its low-level hooks run on a dedicated message loop, never the Office COM worker or application UI thread. Production resume workers read the parent's session-scoped event instead of installing another hook. Input callbacks only latch/forward input; cancellation callbacks run asynchronously outside the hook. Key/button releases and mouse movement are ignored, and no key codes, text or mouse positions are retained.

New input does not cancel the requested initial document open or its read-only observation. It stops later navigation, explicit focus changes and automatic retries, even inside the same Excel window. Unrelated foreground windows also stop these later actions. Permitted HWNDs are checked against their PID and process creation stamp. Exact document observation and position readback are tracked separately from foreground acquisition; a later click cannot erase an already verified result. Office inventory polling uses 250 ms intervals for the first two seconds, then 500 ms within the original deadline. Unconfirmed outcomes remain in history without a repeated unknown-result toast. The app establishes the input baseline after dispatching the initiating Enter/click and hiding its list. Explicit cancellation and the deadline still stop the worker, without undoing an already transmitted COM/Shell action.

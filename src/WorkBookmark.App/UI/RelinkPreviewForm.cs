...
internal sealed class RelinkPreviewForm : Form
{
    public RelinkPreviewForm(Bookmark original, CapturedTarget candidate)
    {
        ...
        string position = candidate.Kind is TargetKind.ExcelCell or TargetKind.WordPosition or TargetKind.PowerPointSlide or TargetKind.PdfPage ? "기존 위치 유지: " + UiStyle.PositionLabel(candidate).Trim(' ', '·') : "대상 종류 유지: " + (candidate.Kind == TargetKind.Folder ? "폴더" : "파일");

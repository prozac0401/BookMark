...
**[WorkBookmark 0.2.4 평가판 다운로드](https://github.com/prozac0401/BookMark/releases/tag/v0.2.4)** — 설정에서 목록과 포스트잇 스티커 중 표시 방식을 선택할 수 있습니다. 스티커의 위치·크기를 조절하고, 지운 책갈피는 최근 삭제에서 복원할 수 있습니다. [변경 사항과 업데이트 안내](docs/release-notes-v0.2.4.md) · [MSI 설치 안내](docs/windows-installer.ko.md).

**0.2.4는 사용자 지정 설치 경로의 복구·제거 문제를 수정했습니다.** 0.2.3에서 추가한 메모 자동 저장과 DB v5는 유지합니다.

**0.2.3부터** 스티커의 메모를 편집하다가 **다른 창이나 스티커로 이동하면 자동 저장**합니다. 저장 버튼은 표시하지 않습니다. 메모 영역을 한 번 클릭하면 그 자리에서 편집하며, **Enter**로 바로 저장하거나 저장 전에 **취소 / Esc**로 되돌릴 수도 있습니다. Release 빌드와 Desktop 회귀검사 209개를 통과하고 합성 화면 4개를 확인했습니다. [검증 결과와 화면](docs/evidence/note-autosave-2026-09-22/README.md)을 확인하세요. 항상 위 표시, 해당 스티커만 처리 표시, 파일 종류 아이콘은 [0.2.1에서 추가한 기능](docs/sticker-fixes-v0.2.1.ko.md)입니다.

설치 패키지는 **서명되지 않은 평가판**입니다. 자동600·MSI 구조50·일반 사용자 설치 수명주기118검사를 통과했습니다. 합성 UI 호스트의 스티커 표시·정상 종료는 확인했으나 실제 마우스 입력·한글 IME·자동 저장·초안·작업 재개, 재로그인·재부팅·여러 모니터 검증은 남아 있습니다. [0.2.4 릴리스 기록](docs/release-notes-v0.2.4.md)을 확인하세요.
...
`WorkBookmark-0.2.4-win-x64.msi`를 실행하면 기본 경로의 이전 MSI 설치를 자동 제거하고 현재 사용자에게 새 버전을 설치합니다. 책갈피와 설정은 유지됩니다. 설치 완료 화면의 **업무 책갈피 실행**은 기본 선택되어 있으며, 바로 실행하지 않으려면 선택을 해제한 뒤 마칩니다. 시작 메뉴에서도 실행할 수 있습니다. Windows 로그인 시 자동 실행은 설정에서 끌 수 있습니다. 제어판 ‘프로그램 제거’ 또는 Windows ‘설치된 앱’에서 제거할 수 있으며 책갈피 데이터는 유지됩니다. 관리자 권한과 SDK 설치는 필요하지 않습니다.
...
0.2.4도 DB v5를 사용하며, 0.2.0~0.2.3에서 업데이트할 때 새 스키마 이전을 추가하지 않습니다.
